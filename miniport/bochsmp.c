    ...
    _Out_ PSTATUS_BLOCK StatusBlock)
{
    ...
    DbgPortLineHex("qemump: MapVideoMemory, physical base ", (unsigned long)DeviceExtension->FrameBuffer.RangeStart.QuadPart);
    DbgPortLineHex("qemump: MapVideoMemory, range length ", DeviceExtension->FrameBuffer.RangeLength);
    ...
    if (Status != NO_ERROR)
    {
        ...
        DbgPortLineHex("qemump: VideoPortMapMemory failed, status ", Status);
    ...
    _Out_ PSTATUS_BLOCK StatusBlock)
{
    ...
    if (ModeRequested >= DeviceExtension->AvailableModeCount)
    {
        ...
        DbgPortLineHex("qemump: mode index out of range ", ModeRequested);
...
BochsFindAdapter(
    ...
    _In_ PUCHAR Again)
{
    DbgPortLine("qemump: BochsFindAdapter");
    ...
    _In_ PVOID HwDeviceExtension)
{
    ...
    DbgPortLine("qemump: BochsInitialize");
...
DriverEntry(PVOID Context1, PVOID Context2)
{
    ...
    DbgPortLine("qemump: DriverEntry");
    ...
    {
        ULONG InitializeStatus = VideoPortInitialize(Context1, Context2, &VideoInitData, NULL);
        DbgPortLineHex("qemump: VideoPortInitialize returned ", InitializeStatus);
